...
/* ===========================
   Soundabode Navbar JS
   - mobile toggle
   - focus trap
   - close on link / esc / outside
   ...
   (function(){
    ...
    const nav = document.querySelector('.sa-navbar');
    const DESKTOP_BREAKPOINT = 1024;
  
    let lastScrollY = window.scrollY;
    let focusableSelectors = 'a[href], button, textarea, input, select, [tabindex]:not([tabindex="-1"])';
    ...
    function setActiveLink(){
      // Highlight current page/section best-effort
      ...
      // If we're on contact.html or /contact
      const isContact = /contact(\.html)?$/.test(path) || path.endsWith('/contact');
      if(isContact){
        links.forEach(a => {
          if(a.getAttribute('href').toLowerCase().includes('contact')) a.classList.add('is-active');
        });
        return;
      }
  
      // Otherwise try to match hash sections on home page
      if(hash){
        links.forEach(a => {
          const href = (a.getAttribute('href') || '').toLowerCase();
          if(href.endsWith(hash)) a.classList.add('is-active');
        });
      } else {
        // default highlight "Home" if we're on index
        const isHome = /index(\.html)?$/.test(path) || path === '/' || path === '';
        if(isHome){
          links.forEach(a => {
            const href = (a.getAttribute('href') || '').toLowerCase();
            if(href === 'index.html' || href === 'index' || href === './' || href === '/') a.classList.add('is-active');
          });
        }
      }
    }
