...
(function () {
  ...
  /* ==========================================================================
     Carousel: measure and clone reliably
     ========================================================================== */
  function getGapValuePx(el) {
    try { const cs = getComputedStyle(el); const gap = cs.getPropertyValue('gap') || cs.getPropertyValue('column-gap') || '0px'; return parseFloat(gap) || 0; } catch (e) { return 0; }
  }

  function measureOneSetWidth(items) {
    if (!items || items.length === 0) return 0;
    let total = 0;
    for (let i = 0; i < items.length; i++) total += items[i].getBoundingClientRect().width;
  ...
  function initCarouselClones() {
    const track = DOM.carouselTrack; if (!track || state.carouselInitialized) return;
    ...
    const wrapper = DOM.carouselSection ? DOM.carouselSection : track.parentElement; if (wrapper) wrapper.style.overflow = wrapper.style.overflow || 'hidden';
    ...
    setTimeout(() => {
      try {
        ...
        clones.forEach((c) => { c.style.flex = c.style.flex || c.getAttribute('data-flex') || ''; track.appendChild(c.cloneNode(true)); });
      ...
      } catch (err) { console.error('initCarouselClones error', err); }
    }, CONFIG.CAROUSEL_CLONE_DELAY);
  }

  function checkCarouselInView() {
    const section = DOM.carouselSection; const track = DOM.carouselTrack; if (!track) return;
    if (!section) { if (!state.carouselInView) { state.carouselInView = true; if (state.carouselInitialized && !state.isCarouselAnimating) startCarouselAnimation(); } return; }
    ...
    if (inView && !state.carouselInView) { state.carouselInView = true; if (state.carouselInitialized && !state.isCarouselAnimating) startCarouselAnimation(); }
    else if (!inView && state.carouselInView) { state.carouselInView = false; stopCarouselAnimation(); }
  }
  ...
  function startCarouselAnimation() {
    ...
    if (!state.carouselOneSetWidth || state.carouselOneSetWidth <= 0) {
      const track = DOM.carouselTrack; const allItems = track ? track.querySelectorAll('.carousel-item') : [];
      if (allItems.length) state.carouselOneSetWidth = measureOneSetWidth(Array.from(allItems).slice(0, Math.max(1, Math.floor(allItems.length / 2))));
      else return;
    }

    state.isCarouselAnimating = true; const track = DOM.carouselTrack; if (!track) return;
    if (state.carouselAnimationId) cAF(state.carouselAnimationId); state.carouselAnimationId = null;

    function step() {
      state.carouselScrollPos += state.carouselSpeed; const w = state.carouselOneSetWidth || (track.scrollWidth / 2) || 0;
      if (w > 0) state.carouselScrollPos = state.carouselScrollPos % w; else state.carouselScrollPos = 0;
  ...
  function stopCarouselAnimation() { if (state.carouselAnimationId) cAF(state.carouselAnimationId); state.carouselAnimationId = null; state.isCarouselAnimating = false; }

  function bindCarouselHoverPause() { const track = DOM.carouselTrack; if (!track) return; track.addEventListener('mouseenter', stopCarouselAnimation, { passive: true }); track.addEventListener('mouseleave', () => { if (state.carouselInView && state.carouselInitialized && state.carouselOneSetWidth > 0) startCarouselAnimation(); }, { passive: true }); }
  ...
  function initTestimonials() {
    ...
    const dotsContainer = DOM.dotsContainer; const prevBtn = DOM.prevBtn; const nextBtn = DOM.nextBtn; const container = DOM.testimonialContainer;
    if (!testimonials.length || !dotsContainer || !prevBtn || !nextBtn || !container) return;

    testimonials.forEach((_, i) => { const d = document.createElement('span'); d.dataset.index = i; if (i === 0) d.classList.add('active'); d.addEventListener('click', () => showTestimonial(i)); dotsContainer.appendChild(d); });

    const dots = Array.from(dotsContainer.querySelectorAll('span'));
    let current = 0;

    function showTestimonial(index) {
      const n = testimonials.length; const newIndex = ((index % n) + n) % n; if (newIndex === current) return;
      testimonials[current].classList.remove('active'); dots[current].classList.remove('active'); testimonials[newIndex].classList.add('active'); dots[newIndex].classList.add('active'); current = newIndex; adjustHeight(); restartAutoSlide();
    }

    function adjustHeight() { const active = container.querySelector('.testimonial.active'); if (!active) return; container.style.height = active.offsetHeight + 'px'; }
    function initHeight() { adjustHeight(); window.addEventListener('resize', adjustHeight, { passive: true }); }
    ...
    function startAuto() { stopAuto(); state.testimonialAutoInterval = setInterval(() => showTestimonial(current + 1), CONFIG.TESTIMONIAL_AUTO_MS); }
    function stopAuto() { if (state.testimonialAutoInterval) clearInterval(state.testimonialAutoInterval); }
    function restartAutoSlide() { stopAuto(); startAuto(); }

    initHeight(); startAuto();
  }
  ...
  function initPopup() {
    ...
    function clearHideTimer() { if (state.popupHideTimer) { clearTimeout(state.popupHideTimer); state.popupHideTimer = null; } }
  ...
  function initPopupFormHandler() {
    const form = DOM.popupForm; if (!form) return;
    ...
    form.addEventListener('submit', async (evt) => {
      ...
      try {
        ...
        const resp = await fetch(BACKEND_URL, {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fullName: name, email: email, phone: phone }), signal: controller.signal
        });
        ...
        if (statusMsg) { statusMsg.textContent = err.name === 'AbortError' ? '❌ Request timed out. Try again.' : '❌ Failed to submit. Please try again.'; statusMsg.style.color = '#ff4444'; }
      } finally {
  ...
  function initContactFormHandler() {
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const submitBtn = form.querySelector('button[type="submit"]');
      const data = { fullName: form.querySelector('#fullName')?.value.trim() || '', email: form.querySelector('#email')?.value.trim() || '', phone: form.querySelector('#phone')?.value.trim() || '', course: form.querySelector('#course')?.value || '', message: form.querySelector('#message')?.value.trim() || '' };

      if (!data.fullName || !data.email || !data.phone) {
        if (statusMsg) { statusMsg.textContent = '❌ Please fill name, email and phone'; statusMsg.style.color = '#ff4444'; }
        return;
      }
      ...
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000);
        const resp = await fetch(BACKEND_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fullName: data.fullName, email: data.email, phone: data.phone, course: data.course, message: data.message, recaptcha: recaptchaResponse }), signal: controller.signal });
        clearTimeout(timeoutId);
        const result = await resp.json().catch(() => ({}));
        if (resp.ok && result && result.success) {
          if (statusMsg) { statusMsg.textContent = "✅ Message sent successfully! We'll get back to you soon."; statusMsg.style.color = '#00ff88'; }
          try { sendConversionTracking('contact_form_submitted', { name: data.fullName, email: data.email }); } catch (e) {}
          form.reset(); if (window.grecaptcha && grecaptcha.reset) grecaptcha.reset();
        } else { const msg = (result && result.message) ? result.message : 'Submission failed'; throw new Error(msg); }
      } catch (err) {
        console.error('Contact submit error', err);
        if (statusMsg) { statusMsg.textContent = err.name === 'AbortError' ? '❌ Request timed out. Try again.' : '❌ Failed to send message. Please try again.'; statusMsg.style.color = '#ff4444'; }
  ...
  function initLogoScroller() {
    ...
    if (!set.children || set.children.length === 0) { console.warn('Logo scroller: no .logo-item children'); return; }

    track.innerHTML = '';
    const set1 = set.cloneNode(true); const set2 = set.cloneNode(true);
    set1.classList.add('logo-set'); set2.classList.add('logo-set'); set1.style.display = 'inline-flex'; set2.style.display = 'inline-flex';
    track.appendChild(set1); track.appendChild(set2);
    track.style.display = 'flex'; track.style.flexWrap = 'nowrap'; track.style.alignItems = 'center';

    function updateLogoSpeed() { const w = window.innerWidth; if (w < 660) state.logoScrollSpeed = CONFIG.LOGO_SCROLL_SPEEDS.xs; else if (w < 768) state.logoScrollSpeed = CONFIG.LOGO_SCROLL_SPEEDS.sm; else if (w < 1440) state.logoScrollSpeed = CONFIG.LOGO_SCROLL_SPEEDS.md; else state.logoScrollSpeed = CONFIG.LOGO_SCROLL_SPEEDS.lg; if (isLowEndDevice()) state.logoScrollSpeed *= 0.6; state.logoSetWidth = 0; }
    function computeLogoSetWidth() { if (!state.logoSetWidth) { const sets = track.querySelectorAll('.logo-set'); if (sets.length > 0) { state.logoSetWidth = Math.round(sets[0].getBoundingClientRect().width); } else state.logoSetWidth = 0; } return state.logoSetWidth; }

    function animateLogo() { if (state.logoAnimationId) cAF(state.logoAnimationId); state.logoAnimationId = null; function step() { if (!state.logoIsPaused) { state.logoCurrentPosition -= state.logoScrollSpeed; const w = computeLogoSetWidth(); if (w > 0) { if (-state.logoCurrentPosition >= w) state.logoCurrentPosition += w; track.style.transform = `translate3d(${state.logoCurrentPosition}px, 0, 0)`; } } state.logoAnimationId = rAF(step); } step(); }

    banner.addEventListener('mouseenter', () => { state.logoIsPaused = true; }, { passive: true }); banner.addEventListener('mouseleave', () => { state.logoIsPaused = false; }, { passive: true });
    const bannerObserver = new IntersectionObserver((entries) => { entries.forEach((entry) => { state.logoIsPaused = !entry.isIntersecting; }); }, { threshold: 0.01 });
    bannerObserver.observe(banner);
    updateLogoSpeed(); window.addEventListener('resize', updateLogoSpeed, { passive: true }); animateLogo();
  }
  ...
  function setupRevealObserver() {
    ...
    const observer = new IntersectionObserver((entries) => { entries.forEach((entry) => { if (entry.isIntersecting) { const elements = entry.target.querySelectorAll('.reveal-up'); elements.forEach((el, i) => { if (!el.classList.contains('active')) setTimeout(() => el.classList.add('active'), i * 100); }); observer.unobserve(entry.target); } }); }, observerOptions);
    DOM.revealSelectors.forEach((selector) => { try { const el = document.querySelector(selector); if (el) observer.observe(el); } catch (e) {} });
  }
  ...
  function initFaqAccordion() { const items = DOM.faqItems; if (!items || !items.length) return; items.forEach((item) => { const head = item.querySelector('.faq-head'); const body = item.querySelector('.faq-body'); if (!head || !body) return; head.addEventListener('click', () => { const open = item.getAttribute('data-open') === 'true'; if (open) { item.setAttribute('data-open', 'false'); head.setAttribute('aria-expanded', 'false'); body.style.maxHeight = body.scrollHeight + 'px'; requestAnimationFrame(() => (body.style.maxHeight = '0')); setTimeout(() => { body.hidden = true; }, 360); } else { item.setAttribute('data-open', 'true'); head.setAttribute('aria-expanded', 'true'); body.hidden = false; requestAnimationFrame(() => (body.style.maxHeight = body.scrollHeight + 'px')); } }); }); }

  function initGearModal() { const cards = DOM.gearCards; const modal = DOM.gearModal; const modalClose = DOM.gearModalClose; if (!cards || !cards.length || !modal || !modalClose) return; function openModal(title, desc, specs) { if (DOM.gearModalTitle) DOM.gearModalTitle.textContent = title || ''; if (DOM.gearModalDesc) DOM.gearModalDesc.textContent = desc || ''; if (DOM.gearModalSpecs) DOM.gearModalSpecs.textContent = specs || ''; modal.setAttribute('aria-hidden', 'false'); document.body.style.overflow = 'hidden'; modalClose.focus(); } function closeModal() { modal.setAttribute('aria-hidden', 'true'); document.body.style.overflow = ''; } cards.forEach((card) => { card.addEventListener('click', () => { const title = card.dataset.title || card.querySelector('.card-title')?.textContent || ''; const desc = card.dataset.desc || card.querySelector('.card-excerpt')?.textContent || ''; const specs = card.dataset.specs || ''; openModal(title, desc, specs); }); }); modalClose.addEventListener('click', closeModal); modal.addEventListener('click', (e) => { if (e.target === modal) closeModal(); }); window.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeModal(); }); }

  function initStudioModal() { if (!DOM.studioModal) return; const cards = Array.from(document.querySelectorAll('#studio-setup .gear-card')); const modal = DOM.studioModal, closeBtn = DOM.studioModalClose; if (!cards.length || !modal || !closeBtn) return; function openModal(title, desc) { if (DOM.studioModalTitle) DOM.studioModalTitle.textContent = title || ''; if (DOM.studioModalDesc) DOM.studioModalDesc.textContent = desc || ''; modal.setAttribute('aria-hidden', 'false'); document.body.style.overflow = 'hidden'; closeBtn.focus(); } function closeModal() { modal.setAttribute('aria-hidden', 'true'); document.body.style.overflow = ''; } cards.forEach((card) => { const title = card.dataset.title; const desc = card.dataset.desc; card.addEventListener('click', () => openModal(title, desc)); card.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openModal(title, desc); } }); const cta = card.querySelector('.card-cta'); if (cta) cta.addEventListener('click', (e) => { e.stopPropagation(); openModal(title, desc); }); }); closeBtn.addEventListener('click', closeModal); modal.addEventListener('click', (e) => { if (e.target === modal) closeModal(); }); window.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeModal(); }); }

  function initWhyDropdown() { const wd = DOM.whyDropdown, toggle = DOM.whyDropdownToggle, panel = DOM.whyDropdownPanel; if (!wd || !toggle || !panel) return; function openPanel() { wd.classList.add('open'); toggle.setAttribute('aria-expanded', 'true'); } function closePanel() { wd.classList.remove('open'); toggle.setAttribute('aria-expanded', 'false'); } function togglePanel(e) { e.preventDefault(); if (wd.classList.contains('open')) closePanel(); else openPanel(); } toggle.addEventListener('click', togglePanel); document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && wd.classList.contains('open')) { closePanel(); toggle.focus(); } }); document.addEventListener('click', (e) => { if (!wd.contains(e.target) && wd.classList.contains('open')) closePanel(); }); }

  /* ==========================================================================
     Global listeners & tracking
     ========================================================================== */
  function bindGlobalListeners() { window.addEventListener('scroll', onScrollHandler, { passive: true }); window.addEventListener('resize', () => { state.carouselOneSetWidth = 0; state.logoSetWidth = 0; state.lastScrollY = window.scrollY || 0; }, { passive: true }); }

  function initBingUET() { (function (w, d, t, r, u) { var f, n, i; w[u] = w[u] || []; f = function () { var o = { ti: '343210550', enableAutoSpaTracking: true }; o.q = w[u]; w[u] = new UET(o); w[u].push('pageLoad'); }; n = d.createElement(t); n.src = r; n.async = 1; n.onload = n.onreadystatechange = function () { var s = this.readyState; s && s !== 'loaded' && s !== 'complete' || (f(), (n.onload = n.onreadystatechange = null)); }; i = d.getElementsByTagName(t)[0]; i.parentNode.insertBefore(n, i); })(window, document, 'script', '//bat.bing.com/bat.js', 'uetq'); }

  function sendConversionTracking(eventName = 'submit_lead_form', payload = {}) { try { if (window.uetq && Array.isArray(window.uetq.push)) window.uetq.push('event', eventName, payload || {}); if (typeof window.uet_report_conversion === 'function') { try { window.uet_report_conversion(); } catch (e) {} } } catch (err) { console.warn('tracking error', err); } }
  ...
  function scheduleInitWhenReady() {
    let called = false; const safeInit = () => { if (!called) { called = true; try { init(); } catch (e) { console.error('init error', e); } } };
    whenStylesAndFontsReady(CONFIG.STYLES_READY_TIMEOUT).then(() => { if ('requestIdleCallback' in window) { try { requestIdleCallback(() => safeInit(), { timeout: 300 }); } catch (e) { setTimeout(safeInit, 0); } } else setTimeout(safeInit, 0); }).catch(() => setTimeout(safeInit, 0));
    setTimeout(() => { if (!called) safeInit(); }, CONFIG.BOOT_TIMEOUT);
  }
