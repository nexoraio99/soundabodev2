/* ============================================================================
   SOUNDABODE SITE SCRIPT - DRIFT-FREE VERSION
   - Fixed animation loop conflicts
   - Proper scroll position normalization
   - Eliminated fractional pixel accumulation
...
(function () {
  ...
  /* ==========================================================================
     CAROUSEL - DRIFT-FREE IMPLEMENTATION
     ========================================================================== */
  ...
  // ✅ FIX: Proper position normalization to prevent drift
  function normalizeCarouselPosition(pos, setWidth) {
    if (!setWidth || setWidth <= 0) return pos;
    
    // Keep position within one set width range [setWidth, 2*setWidth)
    while (pos >= setWidth * 2) {
      pos -= setWidth;
    }
    while (pos < setWidth) {
      pos += setWidth;
  ...
  function initCarouselClones() {
    ...
    setTimeout(() => {
      try {
        ...
        // ✅ FIX: Start at normalized position
  ...
  // ✅ FIX: Completely rewritten animation loop
  function startCarouselAnimation() {
    // Prevent multiple animation loops
    if (state.isCarouselAnimating) {
      console.warn('[Carousel] Already animating, preventing duplicate loop');
      return;
    }
    
    if (!state.carouselInitialized) return;
    ...
    if (state.carouselAnimationId) {
      cAF(state.carouselAnimationId);
      state.carouselAnimationId = null;
    }

    state.isCarouselAnimating = true; 
    state.carouselLastFrameTime = performance.now();
    ...
    function step(currentTime) {
      ...
      // ✅ FIX: Use integer pixel values only
      state.carouselScrollPos += state.carouselSpeed;
      const w = state.carouselOneSetWidth || 0;
      
      if (w > 0) {
        // ✅ FIX: Normalize position every frame to prevent drift
        state.carouselScrollPos = normalizeCarouselPosition(state.carouselScrollPos, w);
      }
      
      // ✅ FIX: Round to integer pixels
      const roundedPos = Math.round(state.carouselScrollPos);
      track.style.transform = `translate3d(-${roundedPos}px, 0, 0)`;
      
      state.carouselLastFrameTime = currentTime;
  ...
  function initCarouselDragSwipe() {
    ...
    function handleStart(e) {
      ...
      // ✅ FIX: Ensure animation is fully stopped
    ...
    function handleMove(e) {
      ...
      // ✅ FIX: Normalize during drag
      if (w > 0) {
        newPos = normalizeCarouselPosition(newPos, w);
        // Update start position if we normalized
        if (Math.abs(newPos - (startScrollPos - deltaX)) > w * 0.5) {
          startScrollPos = newPos + deltaX;
        }
      }
    ...
    function handleEnd() {
      ...
      if (Math.abs(velocity) > 0.5) {
        ...
        // ✅ FIX: Normalize momentum position
        if (w > 0) {
          momentumPos = normalizeCarouselPosition(momentumPos, w);
        }
        ...
        setTimeout(() => {
          ...
          if (state.carouselInView) {
            // ✅ FIX: Small delay before restarting
            setTimeout(() => startCarouselAnimation(), 50);
          }
        }, 600);
      } else {
        if (state.carouselInView) {
  ...
  /* ==========================================================================
     POPUP FORM HANDLER - WITH DUAL SUBMISSION
     ========================================================================== */
  ...
  /* ==========================================================================
     CONTACT FORM HANDLER - WITH DUAL SUBMISSION
     ========================================================================== */
      ...
      function step() {
        if (!state.logoIsPaused) {
          ...
          if (w > 0) {
            ...
            track.style.transform = `translate3d(${Math.round(state.logoCurrentPosition)}px, 0, 0)`;
          }
        }
  ...
  function init() {
    ...
    state.inited = true;
    console.log('[Soundabode] Drift-free version initialized');
