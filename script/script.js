  ...
  */
  ...
  function masterAnimationLoop() {
    ...
    if (state.scrollY !== state.lastScrollY || !state.ticking) {
      ...
      updateIntroOverlay();
      updateMainContent();
      // checkCarouselInView(); // Removed for performance - handled by IntersectionObserver now
    }
  ...
  function startCarouselAnimation() {
    ...
    function step() {
      ...
      track.style.transform = `translate3d(-${Math.max(0, state.carouselScrollPos)}px, 0, 0)`;
  ...
  function initLogoScroller() {
    ...
    function startLogoLoop() {
      if (state.logoAnimationId) return;
    ...
    const bannerObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) startLogoLoop();
        else stopLogoLoop();
      });
    }, { threshold: 0.01 });
  ...
  function ensureAuroraText() {
    ...
    if (!/text/.test(clip) || webkitFill !== 'transparent') {
      el.style.setProperty('background-image', 'linear-gradient(110deg, #3a4045 0%, #768087 35%, #ffffff 45%, #ffffff 50%, #768087 55%, #3a4045 100%)', 'important');
      el.style.setProperty('background-size', '300% auto', 'important');
      el.style.setProperty('-webkit-background-clip', 'text', 'important');
      el.style.setProperty('background-clip', 'text', 'important');
      el.style.setProperty('-webkit-text-fill-color', 'transparent', 'important');
      el.style.setProperty('color', 'transparent', 'important');
      el.style.setProperty('filter', 'drop-shadow(0 4px 6px rgba(0, 0, 0, 0.4))', 'important');
      el.style.setProperty('animation', 'glare-sweep 4s ease-in-out 2 forwards', 'important');
    }
  }
  ...
  function bindGlobalListeners() {
    ...
    // Glare Animation Observer for Section Headings
    if (supportsIntersectionObserver()) {
      const headingObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            entry.target.classList.add('animate-glare');
            headingObserver.unobserve(entry.target); // Run only once
          }
        });
      }, { threshold: 0.5 });

      document.querySelectorAll('.content h2, .produce-content-col h2, .learn-content-col h2').forEach(heading => {
